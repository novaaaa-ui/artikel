<x-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 shadow rounded-xl">
            ...
            <p class="text-4xl font-bold mt-2 text-blue-600">{{ $totalArtikel }}</p>
        </div>

        <!-- Card 2 -->
        <div class="bg-white p-6 shadow rounded-xl">
            <h2 class="text-xl font-semibold text-gray-700">Total Kategori</h2>
            <p class="text-4xl font-bold mt-2 text-green-600">{{ $totalCategory }}</p>
        </div>

        <!-- Card 3 -->
        <div class="bg-white p-6 shadow rounded-xl">
            <h2 class="text-xl font-semibold text-gray-700">Pengguna Terdaftar</h2>
            <p class="text-4xl font-bold mt-2 text-purple-600">{{ Auth::user()->name }}</p>
        </div>
...
<div id="drawer-create-product-default" class="fixed top-0 left-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform -translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
    <h5 id="drawer-label" class="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">New Article</h5>
    ...
    <form action="{{ route('admin.posts.store') }}" method="POST">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type the article title" required="">
            </div>
            <div>
                <label for="author" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Author</label>
                <input name="author_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Author's name" required="">
            </div> 
            <div>
             ...
             <div>
                ...
                <input type="text" name="slug" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type slug" required="">
            </div>
            <div>
                <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Body</label>
                <textarea name="body"id="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Enter the paragraph here"></textarea>
            </div>
